...
- **R-ROOT-NOEXTRA strictness risk:** the report notes only root-`expires` is an *active* db divergence; rejecting unknown `AdditionalProperties` is the strict spec MUST NOT but could reject otherwise-valid cross-stack roots carrying benign extensions. Confirm db's `utils.checkCapability` truly rejects (vs ignores) unknown root fields before making #2 fail-closed on `AdditionalProperties`.
