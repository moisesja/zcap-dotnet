...
| SHOULD-04 | `NormativeIntegrationComplianceTests.Should04_Verifier_RejectsExpirationBeyondThreeMonths_WhenPolicyEnabled` (verifier-side 3-month ceiling, opt-in `VerificationPolicy`, #73); `Should04_Delegation_DoesNotThrowOnLongExpirationAtCreateTime` (create-time no-throw, #61). Path/revocation-bypass behaviour in `VerificationServicePolicyTests`. |
