...
- **Invocation** interop works via Data Integrity `capabilityInvocation` proofs (`SignCapabilityInvocationAsync` / `VerifyCapabilityInvocationAsync`), proven cross-stack for root + delegated. HTTP-Signature invocations (the `ezcap` transport) are not yet supported (tracked in #119).
