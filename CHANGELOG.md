...
- **BREAKING (behaviour)**: the convenience `VerificationService` constructors (2-, 3-, and 4-arg) now default to a fresh `InMemoryNonceStore` instead of `NullNonceStore` (Issue #62; security rationale detailed under **Fixed**). Replay protection is therefore **ON by default**. A 2.x consumer that relied on the previous (insecure) `NullNonceStore` default sees changed behaviour: same-process replays are now rejected, and because `InMemoryNonceStore` is process-local and ephemeral, multi-node or stateless verifiers MUST inject a shared `INonceStore` (a cross-node replay still passes a *different* node, and a process restart clears the seen set). To restore the old behaviour, pass `NullNonceStore.Instance` explicitly to the `INonceStore` constructor overload.
- **BREAKING (API)**: the `IVerificationService` revocation surface changed (Issue #60; detailed under **Added** / **Fixed**). The no-authorization `RevokeCapabilityAsync(string, string)` overload is **removed** and replaced by the authorizing `RevokeCapabilityAsync(Capability, string)`. External implementors of the interface (test doubles, custom verifiers) must update accordingly. The replacement is intentionally a regular abstract member, **not** a C# default interface method: revocation is a security authorization gate, and a throwing/`false`-returning default would let an implementor silently ship without one. The source break is in-policy under SemVer because this release is already a major (3.0.0) bump.
...
- Closed an unauthenticated denial-of-capability gap in revocation (Issue #60). `RevokeCapabilityAsync(string capabilityId, string revokerDid)` performed **no** authorization — it stored the revocation and unconditionally returned `true`, so any caller could revoke anyone's capability (a stored revocation immediately flips `VerifyCapabilityChainAsync`/`VerifyInvocationAsync` to `false`). **BREAKING**: the bare-string overload is removed entirely. The replacement `RevokeCapabilityAsync(Capability, string)` cryptographically verifies the delegation chain before checking authorization, then confirms the revoker controls the capability or any ancestor (up-chain delegator), and returns `false` (recording nothing) when unauthorized — making a `false` return reachable and meaningful. (The `ZcapLd.AspNetCore` revocation endpoint calls `IRevocationService.RevokeAsync` directly and is unaffected.)
