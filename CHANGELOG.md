...
- `VerificationPolicy` (in `ZcapLd.Core.Services`) — opt-in verifier policy for W3C ZCAP-LD SHOULD-level checks not enforced by default (Issue #73). Its `EnforceMaxDelegationExpiration` (default `false`) / `MaxDelegationExpirationMonths` (default `3`) implement the verifier-side SHOULD that an invoked delegated zcap's expiration is not more than three months in the future, measured at verification time. Supplied via the full `VerificationService` constructor, or registered in DI (`services.AddSingleton(new VerificationPolicy { … })` before `AddZcapServices`). This is the correct home for the 3-month ceiling whose create-time hard throw was removed in #61.
...
- `IVerificationService.RevokeCapabilityAsync(Capability, string)` — an authorizing revocation overload (Issue #60). It verifies the revoker controls the capability or any ancestor in its delegation chain (an up-chain delegator) before recording the revocation, and returns `false` (recording nothing) when the revoker is not authorized or the chain cannot be verified. `revokerDid` is a DID the host has already authenticated — the library performs authorization, not authentication.
...
- The invocation-target prefix check now uses an ordinal `StartsWith` (Issue #74). It used the culture-sensitive `string.StartsWith(string)` overload while the following `Substring(capabilityTarget.Length)` is ordinal; with ignorable Unicode code points the two can disagree, and a corner case threw `ArgumentOutOfRangeException` (caught and masked as a spurious rejection). The comparison is now `StartsWith(capabilityTarget, StringComparison.Ordinal)`, consistent with the ordinal index math and free of that exception path. (Impact was minimal — both sites fail closed and real targets are ASCII URIs — but it was a latent correctness/consistency bug.)
- The W3C ZCAP-LD verifier-side 3-month expiration SHOULD is now implemented on the verify path (Issue #73). It previously lived (wrongly) as a create-time hard throw, removed in #61; this adds it where the spec places it — measured at verification time, per delegated chain link, behind the opt-in `VerificationPolicy.EnforceMaxDelegationExpiration` flag (off by default, since it is a SHOULD and can reject legitimately long-lived delegations). The misnamed compliance test now genuinely exercises the verifier: the same long-lived delegation verifies with the policy off and is rejected with it on.
...
- `VerificationService.VerifyInvocationAsync` now enforces `proof.created` freshness (Issue #71). The signed `Proof.Created` timestamp was never checked, so replay was bounded only by the nonce store's TTL — once an entry evicted, the identical captured invocation re-verified (and with replay protection off it was unbounded). The verifier now rejects an invocation whose `proof.created` is missing, dated in the future beyond a one-minute clock-skew allowance, or older than the replay window (`now − created > nonceWindow`), so anything evictable from the nonce store is already too stale to replay. Regression-guarded by `VerificationServiceReplayTests.VerifyInvocation_StaleCreated_IsRejected` / `_FutureCreated_IsRejected` / `_FreshCreated_Verifies`.
- `VerificationService.VerifyCapabilityProofAsync` now enforces attenuation and child expiry against the embedded parent, and its contract is documented as a single-link soundness check rather than a full authorization gate (Issue #69). It verified the delegation signature and parent-controller authorization but did not check that the child stays within the parent (attenuation) or that the child has not expired, so a re-signed child that **expands** authority beyond its parent was accepted by the single-proof method while `VerifyCapabilityChainAsync` correctly rejected it. The shared single-delegation-proof verifier now also rejects an over-broad or expired child (alongside the parent-revocation check added in #63), and the `IVerificationService`/impl XML docs state explicitly that this method checks one delegation link and is **not** an authorization gate — callers wanting an authorization decision should use `VerifyCapabilityChainAsync` / `VerifyInvocationAsync`. Regression-guarded by `VerificationServiceTests.VerifyCapabilityProof_OverbroadChild_ShouldReturnFalse`.
- Verification now binds the resolved key's type to the proof-selected crypto suite (Issue #68). The suite is chosen from the (attacker-controlled) `proof.Type`, but `resolvedKey.KeyType` was never compared against `suite.KeyType`. No forgery was enabled today (the proof type is part of the signed payload, and key importers reject cross-curve bytes), but the missing check was a defense-in-depth gap relying on downstream invariants. Both verify paths now reject early when `suite.KeyType != resolvedKey.KeyType`, self-documenting the invariant and future-proofing against custom resolvers/suites. Regression-guarded by `VerificationServiceTests.VerifyInvocation_WhenResolvedKeyTypeMismatchesSuite_ReturnsFalse`.
- `DidKeyResolver.ResolvePublicKeyAsync` now honours the verification-method `#fragment` instead of always returning the first method (Issue #67). It stripped the fragment and returned `VerificationMethod.FirstOrDefault()`, so for an Ed25519 `did:key` — which resolves to **two** methods (the Ed25519 signing key at index 0 and a derived X25519 key-agreement key at index 1) — resolving the X25519 fragment returned the Ed25519 key, a latent contract violation that worked only by ordering coincidence. When the input carries a `#fragment`, the resolver now selects the method whose `Id` equals the full URI (falling back to a fragment match) and throws `CapabilityValidationException` if none matches, rather than substituting the first; a bare DID still resolves to the primary method. Regression-guarded by `NetDidIntegrationTests.ResolvePublicKeyAsync_X25519KeyAgreementFragment_ReturnsX25519KeyNotEd25519` and `…_UnknownFragment_Throws`.
