...
- [x] `VerificationService.ValidateAttenuation` (~1200): when parent restricts
      allowedAction, require child present AND subset (reject null/empty child).
- [x] `CapabilityService.ValidateAttenuation` (create, ~314): when parent
...
- [x] Add `UriEncoding.EncodeUriComponent` (un-escape %21 %27 %28 %29 %2A).
- [x] Use it in `CapabilityService` root-id derivation (was `Uri.EscapeDataString`).
...
- [x] `LegacyProofCrypto`: always RDFC; drop JCS branch + ValidateCanonicalizationMethod.
- [x] `SigningService`: drop 3-arg ctor + `_canonicalizationMethod`; always RDFC.
- [x] `VerificationService`: drop `canonicalizationMethod` param everywhere; always RDFC.
- [x] `ProofSigningPayloadBuilder`: drop JCS methods (keep RDFC).
- [x] Delete `JcsDocumentCanonicalizer.cs`, `JsonCanonicalizer.cs` (if unused after).
- [x] DI: drop `AddZcapRdfcCanonicalization` + `RdfcCanonicalizationMarker` + resolver; wire RDFC always.
- [x] Tests: delete JCS-only tests; update RdfcEndToEnd/interop/examples to drop param.
- [x] Docs: AGENTS.md, README, architecture, interop report (RDFC is the only mode now).
